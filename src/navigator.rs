    ...
    pub fn handle_action(&mut self, action: Action) -> Result<()> {
        match action {
            ...
            Action::UpdateEpicName { epic_id } => {
                let new_name = (self.prompts.update_name)();
                self.db
                    .update_epic_name(epic_id, new_name)
                    .with_context(|| anyhow!("failed to update epic name!"))?;
            }
            Action::UpdateEpicDescription { epic_id } => {
                let new_description = (self.prompts.update_description)();
                self.db
                    .update_epic_description(epic_id, new_description)
                    .with_context(|| anyhow!("failed to update epic description!"))?;
            }
            ...
            Action::UpdateStoryName { story_id } => {
                let new_name = (self.prompts.update_name)();
                self.db
                    .update_story_name(story_id, new_name)
                    .with_context(|| anyhow!("failed to update story name!"))?;
            }
            Action::UpdateStoryDescription { story_id } => {
                let new_description = (self.prompts.update_description)();
                self.db
                    .update_story_description(story_id, new_description)
            ...
            Action::ReadEpicDescription { epic_id } => {
                self.pages.push(Box::new(EpicDescription {
                    epic_id, 
                    db: self.db.clone()
                }))
            }
            Action::ReadStoryDescription { story_id } => {
                self.pages.push(Box::new(StoryDescription {
                    story_id,
                    db: self.db.clone()
                }))
            }
            Action::ResetDatabase => {
                if (self.prompts.reset_database)() {
                    self.db
                        .reset_database()
                        .with_context(|| anyhow!("failed to reset database!"))?;
                }
            }
            Action::Exit => {
                self.pages.clear()
