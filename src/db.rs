...
impl JiraDatabase {
    ...
    pub fn create_epic(&self, epic: Epic) -> Result<u32> {
        ...
        let updated_id = db_state.last_item_id + 1;

        db_state.last_item_id = updated_id;
        db_state.epics.insert(updated_id, epic);
    ...
    pub fn create_story(&self, story: Story, epic_id: u32) -> Result<u32> {
        ...
        db_state.last_item_id = updated_id;
        db_state.stories.insert(updated_id, story);

        let epic = db_state.epics.get_mut(&epic_id).ok_or_else(|| anyhow!("no epic with this id in database"))?;
        epic.stories.push(updated_id);
    ...
    pub fn delete_story(&self, epic_id: u32, story_id: u32) -> Result<()> {
        ...
        let story_index = epic.stories.iter()
                                            ...
                                            .ok_or_else(|| anyhow!("story id not found in epic stories vec"))?;
    ...
    pub fn update_epic_name(&self, epic_id: u32, name: String) -> Result<()> {
        let mut db_state = self.database.read_db()?;
    ...
    pub fn update_epic_description(&self, epic_id: u32, description: String) -> Result<()> {
        let mut db_state = self.database.read_db()?;
    ...
    pub fn update_story_name(&self, story_id: u32, name: String) -> Result<()> {
        let mut db_state = self.database.read_db()?;

        let story = db_state.stories.get_mut(&story_id).ok_or_else(|| anyhow!("story id not found in epic stories vec"))?;
        story.name = name;
        self.database.write_db(&db_state)?;
        Ok(())
    }

    pub fn update_story_description(&self, story_id: u32, description: String) -> Result<()> {
        let mut db_state = self.database.read_db()?;
    ...
    pub fn reset_database(&self) -> Result<()> {
        let initial_db_state = DBState {
            last_item_id: 0,
            epics: HashMap::new(),
            stories: HashMap::new()
        };
        self.database.write_db(&initial_db_state)?;
        Ok(()) 
    }
}
...
impl Database for JSONFileDatabase {
    fn read_db(&self) -> Result<DBState> {
        let json_contents = std::fs::read_to_string(&self.file_path)?;
        let db_state: DBState = serde_json::from_str(&json_contents)?;
        Ok(db_state)
    }

    fn write_db(&self, db_state: &DBState) -> Result<()> {
        let json_contents = serde_json::to_string(&db_state)?;
        std::fs::write(&self.file_path, json_contents)?;
        Ok(())
    }
}
